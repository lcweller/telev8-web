...
| B19 | **`--color-text-muted` token lightened from `#6e7787` to `#828b99`** in `app/globals.css`. **Footer trademark notice re-tokenized from `--color-text-disabled` to `--color-text-muted`.** | Pre-existing color-contrast failures flagged by Lighthouse a11y audit. (a) `#6e7787` on canvas `#08101f` measured 4.21:1, just under WCAG 2.1 AA's 4.5:1 floor for normal text — new value computes to ~5.5:1, above the floor with margin. Affects every muted-text surface site-wide (hero strap, footer paragraph, footer nav links, etc.) but the perceived lightness shift is small enough to preserve the near-monochrome navy aesthetic per `DESIGN.md` D3. (b) Re-Lighthouse surfaced a second contrast failure on the footer trademark notice (which used `--color-text-disabled` at 2.56:1); the notice is small-print legal copy that needs to remain readable, not a disabled UI element, so the surface was switched to the muted token. The disabled token retains its original value for genuine disabled-state use cases elsewhere. |
| B20 | **Footer column titles demoted from `<h4>` to `<p>`** with the same `eyebrow` class styling. | Pre-existing heading-order failure flagged by Lighthouse a11y audit: footer columns jumped from main-content `<h2>`/`<h3>` straight to `<h4>`, skipping a level. The eyebrow-styled column labels function as section labels, not document headings — demoting to `<p>` removes them from the heading outline entirely (matching the existing `Eyebrow` primitive's choice in `components/primitives/layout.tsx`). Visual rendering unchanged. |
| B21 | **Nav label shortened: `What Doing Nothing Costs` → `Stakes`** in `components/marketing/nav.tsx`. | Static layout analysis showed the 24-char label survived at the 768px tablet breakpoint with only ~21px of width margin — fragile. `Stakes` is 6 chars (~46px), preserves direct meaning of the linked section, and matches the cadence of the other nav labels (Platform / The Plan / Stakes / Team). |
